...
export async function sendMessage(
  ...
  const request = buildApiRequest(context, message, history, sessionId, strategiesDelivered);

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), API_TIMEOUT);

  // If caller provides a cancel signal, forward it to our controller
  if (cancelSignal) {
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      ...
      if (cancelSignal?.aborted) {
        return { ok: false, error: { code: 'network_error', message: 'cancelled' } };
      }
      return {
        ok: false,
        error: {
